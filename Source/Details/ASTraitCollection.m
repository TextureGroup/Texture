...
AS_INLINE UIContentSizeCategory AS_UIContentSizeCategoryUnspecified() {
...
AS_INLINE UIContentSizeCategory _Nonnull AS_safeContentSizeCategory(UIContentSizeCategory _Nullable sizeCategory) {
  return sizeCategory ?: AS_UIContentSizeCategoryUnspecified();
}
...
AS_INLINE NSString *AS_NSStringFromUIUserInterfaceIdiom(UIUserInterfaceIdiom idiom) {
  switch (idiom) {
...
// Named so as not to conflict with a hidden Apple function, in case compiler decides not to inline
AS_INLINE NSString *AS_NSStringFromUIForceTouchCapability(UIForceTouchCapability capability) {
  switch (capability) {
...
// Named so as not to conflict with a hidden Apple function, in case compiler decides not to inline
AS_INLINE NSString *AS_NSStringFromUIUserInterfaceSizeClass(UIUserInterfaceSizeClass sizeClass) {
  switch (sizeClass) {
...
// Named so as not to conflict with a hidden Apple function, in case compiler decides not to inline
AS_INLINE NSString *AS_NSStringFromUIDisplayGamut(UIDisplayGamut displayGamut) {
  switch (displayGamut) {
...
// Named so as not to conflict with a hidden Apple function, in case compiler decides not to inline
AS_INLINE NSString *AS_NSStringFromUITraitEnvironmentLayoutDirection(UITraitEnvironmentLayoutDirection layoutDirection) {
...
#if TARGET_OS_TV
// Named so as not to conflict with a hidden Apple function, in case compiler decides not to inline
AS_INLINE NSString *AS_NSStringFromUIUserInterfaceStyle(UIUserInterfaceStyle userInterfaceStyle) {
