...
- (NSObject *)drawParametersForAsyncLayer:(_ASDisplayLayer *)layer
{
  ...
  return @{
           ...
           @"text": [self l_preparedAttributedText],
...
{
  // First quick check – if the last layout we computed is useable.
  ...
  if ([lastLayout isCompatibleWithContainer:container text:text]) {
    return lastLayout;
  }
  
  // All the keys inside of the cache will have their layout set on them.
  // So the value kind of doesn't matter – if NSCache had a "set" mode
  // where you used the -member: method, then we would use that.
  ...
  ASTextLayout *result = [textLayoutCache objectForKey:key constructedWithBlock:^(ASTextCacheKey *keyParam) {
    return [keyParam createLayout];
  }];
  
  ASLockedSelf(_lastUsedLayout = result);
  return result;
}
...
+ (void)drawRect:(CGRect)bounds withParameters:(NSDictionary *)layoutDict isCancelled:(asdisplaynode_iscancelled_block_t)isCancelledBlock isRasterizing:(BOOL)isRasterizing
{
  ...
  // We try to limit our use of instance, but we should update
  // its _lastUsedLayout to get MAXIMUM PERFORMANCE
  ASTextLayout *layout = [layoutDict[@"instance"] compatibleLayoutWithContainer:container text:text];
                 ...
                 forHighlighting:(BOOL)highlighting
{
  ...
  ASTextLayout *layout = [self compatibleLayoutWithContainer:containerCopy text:_attributedText];
